...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(
        ...
        try:
            ...
            if os.path.isfile(path):  # file
                with open(path, "r") as f:
                    f = f.read().splitlines()
                    f = [
                        x.replace("./", parent) if x.startswith("./") else x for x in f
        ...
        np_labels_path = (
            str(Path(self.label_files[0]).parent) + ".npy"
        ...
        for i, file in enumerate(pbar):
            ...
            pbar.desc = (
                "Caching labels %s (%g found, %g missing, %g empty, %g duplicate, for %g images)"
                % (s, nf, nm, ne, nd, n)
            )
...
def random_affine(
    img, targets=(), degrees=10, translate=0.1, scale=0.1, shear=10, border=(0, 0)
):
    ...
    T = np.eye(3)
    T[0, 2] = (
        random.uniform(-translate, translate) * img.shape[1] + border[1]
    )  # x translation (pixels)
    T[1, 2] = (
        random.uniform(-translate, translate) * img.shape[0] + border[0]
