...
def create_pretrained(
    f="weights/best.pt", s="weights/pretrained.pt"
...
def plot_one_box(x, img, label=None, line_thickness=None):
    ...
    c2 = c1[0] + 50, c1[1] - t_size[1] - 3
    cv2.rectangle(img, c1, c2, color, -1, cv2.LINE_AA)  # filled
    pil_img = Image.fromarray(img)
    font = ImageFont.truetype("utils/YaHei.ttf", 20)
    fillColor = (225, 255, 255)
    position = (c1[0], c1[1] - 25)
    draw = ImageDraw.Draw(pil_img)
    draw.text(position, "石头", font=font, fill=fillColor)
...
def plot_results(
    start=0, stop=0, bucket="", id=(), labels=()
