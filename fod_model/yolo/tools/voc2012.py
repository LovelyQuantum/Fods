...
data_dir = Path("/home/hina/research/yqfods/dataset/voc_mode/")
output_file = Path("data/voc2012_val.tfrecord")
classes = Path("data/voc_mode.names")
split = "val"


def build_example(annotation, class_map):
    img_path = Path.joinpath(data_dir, "JPEGImages", annotation["filename"])
    ...
    if "object" in annotation:
        for obj in annotation["object"]:
            ...
            classes_text.append("stone".encode("utf8"))
            classes.append(class_map["stone"])
            ...
            feature={
                ...
                "image/filename": tf.train.Feature(
                    bytes_list=tf.train.BytesList(
                        value=[annotation["filename"].encode("utf8")]
                    )
                ),
...
def main():
    ...
    logging.warning("Class mapping loaded: %s", class_map)

    writer = tf.io.TFRecordWriter(str(output_file))
    image_list = (
        open(Path.joinpath(data_dir, "ImageSets", "Main", "%s.txt" % split))
        .read()
        .splitlines()
    )
    logging.warning("Image list loaded: %d", len(image_list))
    for image in tqdm(image_list):
        name = Path(image).stem
        annotation_xml = Path.joinpath(data_dir, "Annotations", name + ".xml")
